...
\titleformat{\chapter}[display]
  {\normalfont\Large\bfseries\color{MidnightBlue}}{\chaptertitlename\ \thechapter}{8pt}{\LARGE\bfseries}
\titleformat{\section}
...
\begin{tikzpicture}[node distance=1.2cm, auto, >=Latex, 
    block/.style={rectangle, draw=NavyBlue, fill=NavyBlue!8, text width=2.6in, text centered, rounded corners, minimum height=0.5in, font=\small\bfseries},
    subblock/.style={rectangle, draw=DarkSlateGray, fill=white, text width=2.4in, rounded corners, minimum height=0.4in, font=\footnotesize}]
    
    \node [block] (root) {Unit 1: One-Variable Data (15--23\%)};
    \node [subblock, below=0.6cm of root] (data) {\textbf{Data Types:} Categorical (Bar/Pie) vs Quantitative (Dot/Stem/Hist/Boxplot)};
    \node [subblock, below=0.5cm of data] (socs) {\textbf{Description Protocol:} SOCS (Shape, Outliers, Center, Spread) in context};
    \node [subblock, below=0.5cm of socs] (outliers) {\textbf{Outlier Rule:} Lower $= Q_1 - 1.5\text{IQR}$, Upper $= Q_3 + 1.5\text{IQR}$};
    \node [subblock, below=0.5cm of outliers] (norm) {\textbf{Normal Model:} $z = \frac{x-\mu}{\sigma}$, 68-95-99.7 Rule, \texttt{normalcdf}};
...
\begin{tikzpicture}[node distance=1.1cm, auto, >=Latex,
    block/.style={rectangle, draw=NavyBlue, fill=NavyBlue!8, text width=2.6in, text centered, rounded corners, minimum height=0.45in, font=\small\bfseries},
    subblock/.style={rectangle, draw=DarkSlateGray, fill=white, text width=2.4in, rounded corners, minimum height=0.4in, font=\footnotesize}]
    
    \node [block] (root) {Unit 2: Two-Variable Data (5--7\%)};
    \node [subblock, below=0.5cm of root] (scatter) {\textbf{Scatterplots:} Direction ($+/-$), Form (linear/non), Strength, Outliers};
    \node [subblock, below=0.4cm of scatter] (lsrl) {\textbf{LSRL Line:} $\hat{y} = a + bx$ (Always include hat on predicted $y$)};
    \node [subblock, below=0.4cm of lsrl] (res) {\textbf{Residuals:} $\text{Residual} = y - \hat{y}$ (Actual minus Predicted)};
    \node [subblock, below=0.4cm of res] (r2) {\textbf{Coefficients:} Correlation $r$, Coefficient of Determination $r^2$, SE $s$};
...
\begin{tikzpicture}[node distance=1.1cm, auto, >=Latex,
    ...
    \node [subblock, below=0.5cm of root] (obs) {\textbf{Observational Studies:} Shows association only; no treatments assigned.};
    \node [subblock, below=0.4cm of obs] (exp) {\textbf{Experiments:} Random assignment of treatments; can establish \textbf{Causation}.};
    \node [subblock, below=0.4cm of exp] (sample) {\textbf{Sampling Methods:} SRS, Stratified (homogeneous), Cluster (heterogeneous).};
    \node [subblock, below=0.4cm of sample] (bias) {\textbf{Bias Types:} Undercoverage, Nonresponse, Response, Voluntary Response.};
...
\textbf{3. Independence Test:} Events $A$ and $B$ are independent if and only if $P(A \mid B) = P(A)$ or $P(A \cap B) = P(A) \cdot P(B)$.

\textbf{4. Combining Random Variables:}
$$\mu_{X \pm Y} = \mu_X \pm \mu_Y \qquad \sigma_{X \pm Y}^2 = \sigma_X^2 + \sigma_Y^2 \quad \text{(\textbf{Always ADD variances!})}$$
...
$$\mu_{\hat{p}} = p \qquad \sigma_{\hat{p}} = \sqrt{\frac{p(1-p)}{n}} \quad (\text{Normal if } np \ge 10, n(1-p) \ge 10)$$

\textbf{2. Sample Mean ($\bar{x}$):}
$$\mu_{\bar{x}} = \mu \qquad \sigma_{\bar{x}} = \frac{\sigma}{\sqrt{n}} \quad (\text{Normal if } n \ge 30 \text{ by CLT or pop is normal})$$
\end{formulabox}
...
$$\hat{p}_c = \frac{x_1 + x_2}{n_1 + n_2} \qquad z = \frac{(\hat{p}_1 - \hat{p}_2) - 0}{\sqrt{\hat{p}_c(1-\hat{p}_c)\left(\frac{1}{n_1} + \frac{1}{n_2}\right)}}$$
\end{formulabox}
...
$$\chi^2 = \sum \frac{(\text{Observed} - \text{Expected})^2}{\text{Expected}} \qquad \text{Expected Count} = \frac{(\text{Row Total}) \times (\text{Column Total})}{\text{Table Total}}$$
\end{formulabox}
...
\centering
\scriptsize
\begin{tabularx}{\linewidth}{@{}llllp{1.4in}@{}}
\toprule
